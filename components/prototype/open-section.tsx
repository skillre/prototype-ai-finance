import { SceneBackdrop } from "@/lib/kits/adapters/scene"
...
type OpenSectionProps = {
   ...
   * corners to keep the wash from bleeding past its own bounds.
   */
  className?: string
 ...
 *
 * 参数档位由适配层决定（见 lib/kits/adapters/scene.tsx）：
 *   hero  → 疏网格 + 强淡出 + 极缓漂移（"这个数字在空间里"）
 *   wash  → 密网格 + 弱淡出 + 静止（面板是阅读容器，背板不该动）
 *
 * 注意 <SceneBackdrop /> 自己声明 `aria-hidden`（它知道自己是装饰），
 * 因此这里不再重复加 —— 加了反而编译不过。
 *
 * ---------------------------------------------------------------------------
 * surface：主视觉的"面"
      ...
      {ambient !== "none" ? (
        <div
          ...
          className={cn(
            ...
            surface ? "bg-surface shadow-floating" : ambient === "hero" ? "bg-[var(--hero-base)]" : "bg-surface/40"
          )}
        >
