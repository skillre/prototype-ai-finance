...
| 入口 | 模式 | 跑在 | 扫描对象 |
|---|---|---|---|
| `pnpm qa` | **LOCAL_MANAGED** | `.qa/browser-qa.mjs` | 本 run 自己起的 dev server（端口 3210） |
...
| 规则 | 实现 |
|---|---|
| 不复用 3000 | `.qa/qa.config.mjs` 的 `QA_PORT = 3210`（本产品的槽位，不是 Factory 的 3200） |
...
|---|---|---|
| origin | 本 run 起的 `127.0.0.1:3210` | `--base-url` |
