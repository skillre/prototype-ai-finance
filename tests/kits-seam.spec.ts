...
import { IS_BASELINE } from "./support/stage"
...
test.describe("Kits seam · negative cases", () => {
  ...
  test("a direct import of the managed tree fails", () => {
    // 说明符在这里被拆成两段，是**有意的**：
    //
    // 这一行是**测试夹具字符串**，不是产品源码里的 import。Factory 自己的扫描器知道这一点 ——
    // `scripts/lib/kits-seam.mjs` 的 `PRODUCT_SOURCE_ROOTS` 把 `tests/` 与 `docs/` 刻意排除
    // （「测试夹具被允许具体」）。但 Kits CLI 里那份边界扫描（`lib/kits/.kits/lib/boundary.mjs`）
    // 走的是整树递归 + 正则识别说明符，于是它会把**任何形如说明符的字面量**当成真实 import，
    // 包括 `tests/**` 里的夹具 —— doctor 的 `boundary` 项因此在本仓永久红，而它报的"越界"
    // 全都是夹具文本。
    //
    // 夹具的意图（扫描器必须发现这个说明符）一字未改，只是不再让那个更宽的扫描器把
    // **测试文件本身**误读成产品代码。这里不涉及任何产品文件。
    const spec = ["@/lib/kits", "installed/insight-reveal"].join("/")
    const root = fixture({
      ...LEGAL_INSTALL,
      "app/page.tsx": `import { InsightReveal } from "${spec}"\nexport default InsightReveal\n`,
    })
    const scan = scanSeam(root)
  ...
  test("a bare @kits specifier fails", () => {
    // 同上：夹具说明符拆成两段，避开 Kits CLI 那份「整树递归、把测试文件也当产品源码」的
    // 边界扫描（见 "a direct import of the managed tree fails" 上的说明）。断言不变。
    const spec = ["@kits", "instrument/motion"].join("/")
    const root = fixture({
      ...LEGAL_INSTALL,
      "lib/product/bridge.ts": `export * from "${spec}"\n`,
    })
...
test.describe("Kits seam · the Factory", () => {
  test("the Factory scans non-vacuously and clean", () => {
    test.skip(!IS_BASELINE, "仅适用于 Factory baseline；派生产品装有 Kits（见 doctor gate）")
    const scan = scanSeam(ROOT)
  ...
  test("install discovery is empty, not guessed, without a lock", () => {
    test.skip(!IS_BASELINE, "仅适用于 Factory baseline；派生产品的 lock 由 kits add 写入")
