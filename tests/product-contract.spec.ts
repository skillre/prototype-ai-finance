...
test.describe("no generated semantics", () => {
  ...
  test("the shipped contract declares this product's own invariants", () => {
    // Inverse of the baseline's rule, on purpose. In `prototype-starter` a domain
    // invariant would be inherited residue; here it is the whole point — this
    // repository IS a product, and `product-contract.json` is where its semantic
    // invariants live (the three ledger identities).
    //
    // What must *not* survive is the other direction: a baseline-level `factory.*`
    // invariant inherited without the core spec that holds it (this product does
    // not ship `tests/core-neutrality.spec.ts`).
    ...
    // The gate's own invariant: registered by this very spec file, so the contract
    // has to declare it or the bidirectional check reports `contract/undeclared`.
    expect(ids).toContain("contract.declaration-matches-enforcement")

    // Domain invariants must be this product's own.
    expect(ids.filter((id: string) => id.startsWith("ledger.")).length).toBeGreaterThan(0)

    // Baseline-only invariants whose enforcement spec was not carried over.
...
/**
 * In `prototype-starter` this section read the third prototype **read-only**, from
 * an absolute path outside the repo: it was the evidence that the contract was
 * expressive enough for a real product's invariants — at that time they had no
 * home outside `tests/invariants.spec.ts` (F11).
 *
 * This repository **is** that product: the three ledger identities already live in
 * `product-contract.json`. So the same probe now runs against the product's own
 * spec, and it can make one claim the read-only version could not:
 * **every `ledger.*` declaration has its registration inside the very file that
 * holds the assertions.** The global scanner only knows "somewhere, a test
 * registered this id"; this pins it to the file that carries the equality.
 */
test.describe("本产品的领域不变量住在真实断言里", () => {
  /** The spec that holds the three ledger identities. */
  const SPEC = "tests/finance-data.spec.ts"

  test("每条 ledger.* 声明都能在账本 spec 里找到同 id 的登记", () => {
    const contract = JSON.parse(read(CONTRACT_FILENAME))
    const declared: string[] = contract.invariants
      .map((entry: { id: string }) => entry.id)
      .filter((id: string) => id.startsWith("ledger."))

    // Zero would mean the contract degraded to mechanism-level invariants only.
    expect(declared.length).toBeGreaterThan(0)

    /*
     * 用的是**门禁自己的那份正则**（`REGISTRATION_PATTERN`），不是这里重写一遍的
     * 字符串匹配：`invariant(` 与 id 之间允许换行，而字符串 `includes` 会漏掉那种写法，
     * 于是断言会以"实现不同"的方式悄悄与门禁脱钩。
     */
    const registered = new Set(
      [...stripComments(read(SPEC)).matchAll(REGISTRATION_PATTERN)].map((match) => match[1]),
    )

    for (const id of declared) {
      // 断言消息里的调用名同样拆开写：这个文件也在扫描器的作用域里，
      // 一句字面量 `invariant("…")` 会被读成一次登记（扫描器处理注释，不处理字符串内容）。
      const call = ["inv", "ariant("].join("")
      expect(registered.has(id), `${id} 在 ${SPEC} 里没有对应的 ${call}"${id}") 登记`).toBe(true)
    }
  })

  test("这些 id 都是机器可读的，且没有 baseline 残留", () => {
    const contract = JSON.parse(read(CONTRACT_FILENAME))
    const ids: string[] = contract.invariants.map((entry: { id: string }) => entry.id)
    for (const id of ids) expect(id).toMatch(ID_PATTERN)
    expect(ids.filter((id) => id.startsWith("ledger.")).length).toBeGreaterThan(0)
    expect(ids.filter((id) => id.startsWith("factory."))).toEqual([])
  })
})
